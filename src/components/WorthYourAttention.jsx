...
const SIGNALS = [
  ...
  {
    x: 485, y: 280, w: 245, h: 76, fill: "#123d2f", stroke: "#4ecb74",
    iconX: 515, icon: "message", iconTone: "#8ae887",
    tx: 558, lines: [["Customer Sentiment", 310], ["Shift", 335]],
  },
  {
...
const CONVERGE = [
  ...
  { d: "M608 356 C628 475,655 600,690 720", stroke: "#66c879", width: 2.0, pulse: "#79e08b", r: 7 },
  ...
  <figure className="product-frame wya">
    <svg viewBox="0 0 1380 900" role="img" aria-labelledby="wya-title wya-desc">
      ...
      <defs>
        ...
        <radialGradient id="wya-attention-glow">
          <stop offset="0%" stopColor="#a77cff" stopOpacity=".95" />
          <stop offset="48%" stopColor="#5b24d4" stopOpacity=".78" />
          <stop offset="100%" stopColor="#5b24d4" stopOpacity="0" />
        </radialGradient>
      ...
      <g className="wya-header" fontFamily="Inter, Arial, sans-serif">
        <text x="690" y="58" textAnchor="middle" fontSize="13" fontWeight="800" fill="#8879d6" letterSpacing="1.6">
          EVERYTHING HAPPENING IN YOUR MARKET
        </text>
      </g>
      ...
      {/* The nine that turn out to matter, brightening one after another. */}
        ...
        {SIGNALS.map((signal, i) => (
          <g className={`wya-signal wya-signal-${i + 1}`} key={signal.icon}>
            ...
            {signal.lines.map(([line, baseline]) => (
              <text key={line} x={signal.tx} y={baseline} fontSize="17" fontWeight="800" fill="#fff">
                {line}
              </text>
            ))}
          </g>
      ...
      <g className="wya-attention">
        <ellipse cx="690" cy="755" rx="190" ry="62" fill="url(#wya-attention-glow)" filter="url(#wya-glow)" />
        <ellipse cx="690" cy="755" rx="138" ry="39" fill="#39198e" opacity=".93" />
        <circle className="wya-check" cx="690" cy="724" r="24" fill="#fff" fillOpacity=".08" stroke="#ae93ff" strokeWidth="2" />
        <text x="690" y="733" textAnchor="middle" fontSize="25" fill="#fff">
          ✓
        </text>
        <text x="690" y="786" textAnchor="middle" fontFamily="Inter, Arial, sans-serif" fontSize="22" fontWeight="850" fill="#fff">
